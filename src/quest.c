...
#define QUEST_ITEM1		8326
#define QUEST_ITEM2		8327
#define QUEST_ITEM3		8328
#define QUEST_ITEM4		8329
#define QUEST_ITEM5		8330
#define QUEST_ITEM6		8331
#define QUEST_ITEM7		8332
#define QUEST_ITEM8		8333
#define QUEST_ITEM9		8334
#define QUEST_ITEM10	8335
#define QUEST_ITEM11	10006
#define QUEST_ITEM12	8336
#define QUEST_ITEM13	8337
#define QUEST_ITEM14	8338
...
#define QUEST_OBJQUEST1 8322
#define QUEST_OBJQUEST2 8323
#define QUEST_OBJQUEST3 8324
...
void do_quest(CHAR_DATA *ch, char *argument)
{
	...
	if (ch->level > 101)
	{
		send_to_char("No, just load the freaking obj you wanker",ch);
		return;
	}
...
/* And, of course, you will need to change the following lines for YOUR
   ...
   build up quest points :> Make the item worth their while. */
	...
	sprintf(buf, "{MQuest Items {Cavailable for Purchase:{x\n\r\
{B1000{Gqp{W.........{xThe {DB{Rl{Do{Ro{Dd-{yL{Ya{yn{Yc{ye{x (Two-handed Spear)\n\r\
{B1000{Gqp{W.........{YC{Dh{Ra{Yl{Dd{Ra{Yh{Do{Rg {xthe '{rD{De{rm{Do{rn {DS{rl{Da{ry{De{rr{x'( One-handed sword)\n\r\
{B1000{Gqp{W.........{xThe {rG{Do{rr{De{rS{Dh{ro{Dv{re{Dl{x (Two-handed axe)\n\r\
{B1000{Gqp{W.........{xThe {WC{Dr{Wa{Dn{Wi{Da{Wl {RN{ce{Rm{ce{Rs{ci{Rs{x (One-handed mace)\n\r\
{B1000{Gqp{W.........{xThe {RS{rp{yi{rn{Re{yR{ri{Rpp{re{yr{x (One-handed dagger)\n\r\
{B1000{Gqp{W.........{xThe {GS{ct{go{cr{Gm{Cs{Gt{cr{gi{ck{Ge{x (Two-handed bow)\n\r\
{B1000{Gqp{W.........{xThe {GV{gi{Rp{re{Gr{y'{Rs {gC{Go{yi{Rl{x (One-handed whip)\n\r\
{B500{Gqp{W..........{CT{Bea{Cr {xof the {YG{rod{Ys{x (Necklace)\n\r\
{B500{Gqp{W..........{YM{Da{Yr{Dk {xof the {RB{me{Ra{ms{Rt{x (Ring)\n\r\
{B750{Gqp{W..........{ySw{wor{Dd{wba{yck {wH{Dol{wd{x (Shield)\n\r\
{B2500{Gqp{W.........{WMask {wof {gA{Yg{Do{Yn{gy{x (Mask)\n\r\
{B500{Gqp{W..........{G5 {Wlbs of {cSt{ca{Yrf{Ci{cre {WOre{x (Ore)\n\r\
{B500{Gqp{W..........{xa {Rt{yo{Yr{yc{Rh{x (Light)\n\r\
{B200{Gqp{W..........{x200,000 gold pieces (Moron)\n\r\
{B200{Gqp{W..........{x30 Practices (Ask and die)\n\r\
	{CTo buy an item, type {w'{GQUEST BUY {D<{Bitem{D>{w'{C.{x\n\r");
	send_to_char(buf, ch);
	return;
    }
	...
	if (is_name(arg2, "amulet tear gods"))
	{
	...
	else if (is_name(arg2, "shield swordback hold"))
	{
	...
	else if (is_name(arg2, "mask agony"))
	{
	    if (ch->questpoints >= 2500)
	    {
			ch->questpoints -= 2500;
			obj = create_object(get_obj_index(QUEST_ITEM14),ch->level);
			sprintf(buf,"%s is buying a MASK!\n\r",ch->name);
			append_new_test(LOG_FILE,buf);
	    }
	    else
	    {
			sprintf(buf, "Sorry, %s, but you don't have enough quest points for that.",ch->name);
	...
	else if (is_name(arg2, "sword chaldahog demon slayer") )
	{
	...
	else if (is_name(arg2, "axe goreshovel shovel") )
	{
	    if (ch->questpoints >= 1000)
	    {
			ch->questpoints -= 1000;
	...
	else if (is_name(arg2, "cranial cranium nemesis mace") )
	{
	    if (ch->questpoints >= 1000)
	    {
			ch->questpoints -= 1000;
	...
	else if (is_name(arg2, "stormstrike strike bow") )
	{
	    if (ch->questpoints >= 1000)
	    {
			ch->questpoints -= 1000;
	...
	else if (is_name(arg2, "vipers coil whip") )
	{
	    if (ch->questpoints >= 1000)
	    {
			ch->questpoints -= 1000;
	...
	else if (is_name(arg2, "spine ripper dagger spineripper") )
	{
	    if (ch->questpoints >= 1000)
	    {
			ch->questpoints -= 1000;
	...
	else if (is_name(arg2, "blood lance blood-lance spear"))
	{
	    if (ch->questpoints >= 1000)
	    {
			ch->questpoints -= 1000;
	...
	else if (is_name(arg2, "ore starfire") )
	{
	    if (ch->questpoints >= 500)
	    {
			ch->questpoints -= 500;
	...
	else if (is_name(arg2, "mark beast tiger"))
	{
	    if (ch->questpoints >= 500)
	    {
		ch->questpoints -= 500;
	...
	else if (is_name(arg2, "torch light lantern") )
	{
	    if (ch->questpoints >= 500)
	    {
			ch->questpoints -= 500;
	...
	else if (is_name(arg2, "gold gp"))
	{
	    if (ch->questpoints >= 200)
	    {
			...
			ch->questpoints -= 200;
			ch->gold += 200000;
			act( "$N gives 200,000 gold pieces to $n.", ch, NULL, questman, TO_ROOM );
			act( "$N has 200,000 in gold transfered from $s Swiss account to your balance.",   ch, NULL, questman, TO_CHAR );
			sprintf(bug,"%s quest bought gold at: %s",ch->name,(char *) ctime( &current_time ));
			append_file( ch, BUG_FILE, bug );
	        return;
	    }
	    else
	    {
	...
	if (IS_NPC(ch))
	{
		sprintf(buf, "I can't give YOU a quest, you're an NPC!");
		do_say(questman,buf);
		return;
	}
	if (ch->nextquest > 0)
	...
	if (IS_SET(ch->act, PLR_QUESTOR))
	{
	    if (ch->questmob == -1 && ch->countdown > 0)
	    {
			...
			pointreward = number_range(15,25);
			...
			ch->nextquest = 5;
			ch->gold += reward;
			add_qp(ch, pointreward);
			ch->pcdata->gamestat[QUESTS_COMPLETE]++;
			//stc("Adding one to gamestat[QUESTS_COMPLETE]\n\r",ch);
			return;
		}
		...
		if (obj_found == TRUE)
		{
		    ...
		    pointreward = number_range(25, 30);
    ...
    {
		sprintf(buf, "You give your quest up, you are a failure");
		send_to_char(buf, ch);
		REMOVE_BIT(ch->act, PLR_QUESTOR);
		ch->questgiver = NULL;
		ch->countdown = 0;
...
void generate_quest(CHAR_DATA *ch, CHAR_DATA *questman)
{
	...
	{
		room = find_location( ch, vsearch->short_descr );
    ...
    {
		sprintf(buf, "I'm sorry, I cant seem to find the person I wanted to.");
		do_say(questman, buf);
		sprintf(buf, "Try again later.");
		do_say(questman, buf);
		ch->nextquest = 2;
	...
	case 1:
	sprintf(buf, "Midgaard's most heinous criminal, %s, has escaped from the dungeon!",victim->short_descr);
	do_say(questman, buf);
...
{
    if (clevel < 6 && mlevel < 10) return TRUE;
    else if (clevel > 6 && clevel < 20 && mlevel < 40) return TRUE;
    else if (clevel > 19 && clevel < 60 && mlevel > 45 && mlevel < 75) return TRUE;
    else if (clevel > 59 && clevel < 75 && mlevel > 75 && mlevel < 90) return TRUE;
    else if (clevel > 74 && clevel  < 102 && mlevel > 70 && mlevel < 102) return TRUE;
    else if (clevel > 101 && mlevel > 101) return TRUE;
    else return FALSE;
}
...
void add_qp( CHAR_DATA *ch,int amount )
{
	char buf[MSL];
	if(ch->clan != 0 
		&& clan_table[ch->clan].clan_points < 0)
	{
		sprintf(buf,"{RBut you must donate all %d {CQPs{R to your clan, becuase your cache is negative{x\n\r",
			amount);
		stc(buf,ch);
		return;
	}
