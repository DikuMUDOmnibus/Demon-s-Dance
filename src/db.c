...
/*draconian*/
sh_int			gsn_find_dragon;
sh_int			gsn_sever;
/*sensei*/
sh_int			gsn_haduken;
sh_int			gsn_triple_kick;
sh_int			gsn_shadow_slip;
sh_int			gsn_fatality;
/*Zealot*/
sh_int			gsn_smite;
/*Dagashi*/
sh_int			gsn_burst_of_speed;
sh_int			gsn_coat;
sh_int			gsn_dagashi_poison;
sh_int			gsn_gouge;

sh_int			gsn_create_golem;
...
void boot_db( void )
{
  ...
  /* OLC */     
	...
	load_notes( );
	/*load_bans();*/
	append_new_save(NEW_SAVE_TEST,"Done loading bans\n\r");
	load_songs();
	load_disabled();
	clear_raid_info( );	
	if (get_obj_index( 11022 ) != NULL)
	{
		obj = (get_obj_index( 11022 ));
	}
    }
    return;
}
...
/* */
		...
		case ITEM_GOLEM_PART:	olevel = number_range(  5, 15 ); break;
		...
		if ( pObjIndex->count < limit || number_range(0,4) == 0 )
		{
		    ...
		    if (pObj->level > LastMob->level + 30
		    ||  (pObj->item_type == ITEM_WEAPON 
		    &&   pReset->command == 'E' 
		    &&   pObj->level < LastMob->level - 30))
			fprintf(stderr,
			    "Err: obj %s (%d) -- %d, mob %s (%d) -- %d\n",
			    pObj->short_descr,pObj->pIndexData->vnum,pObj->pIndexData->level,
...
CHAR_DATA *create_mobile( MOB_INDEX_DATA *pMobIndex )
{
	...
	mob->teach_flags	= pMobIndex->teach_flags;
	...
	mob->clan               = pMobIndex->clan;  /* Feydrex - clan guard */
...
void clone_mobile(CHAR_DATA *parent, CHAR_DATA *clone)
{
    ...
    clone->teach_flags	= parent->teach_flags;
...
OBJ_DATA *create_object( OBJ_INDEX_DATA *pObjIndex, int level )
{
	...
	obj->orig_short		= str_dup( pObjIndex->orig_short);
