...
void do_put( CHAR_DATA *ch, char *argument )
{
    ...
    if ( (container->item_type != ITEM_CONTAINER) && (container->item_type != ITEM_GOLEM_BAG) )
    {
	send_to_char( "That's not a container.\n\r", ch );
	return;
    }
...
void do_give( CHAR_DATA *ch, char *argument )
{
	...
	if ( !silver && victim->spec_fun == spec_lookup("spec_questmaster"))
	{
		qp_give = amount/1000;
		sprintf(buf,"%s gives you %d QPs for your donation",victim->short_descr,qp_give);
		send_to_char(buf,ch);
		add_qp(ch,qp_give);
	}
...
void do_eat( CHAR_DATA *ch, char *argument )
{
	...
	case ITEM_TOKEN:
	{
		if ( !IS_NPC(ch) )
		{
		sprintf(buf,"{WYou gain {C%d{W QP's{x",obj->value[0]);
		send_to_char(buf,ch);
		add_qp(ch,obj->value[0]);
		}
		break;
	}
	}
...
bool remove_obj( CHAR_DATA *ch, int iWear, bool fReplace )
{
	...
	{
		stc("You have to dislodge the arrow, you cant just remove it\n\r",ch);
		return FALSE;
	}
...
void wear_obj( CHAR_DATA *ch, OBJ_DATA *obj, bool fReplace )
{
    ...
    if ( CAN_WEAR( obj, ITEM_WEAR_NOCKED ) )
    {
	if ( !remove_obj( ch, WEAR_NOCKED, fReplace ) )
	    return;
	act( "$n nocks $p on $s bow.",   ch, obj, NULL, TO_ROOM );
	act( "You nock $p to your on.", ch, obj, NULL, TO_CHAR );
...
/* shield section */
    ...
    if ( CAN_WEAR( obj, ITEM_WIELD )  )
    {
		...
		if (IS_WEAPON_STAT(obj,WEAPON_TWO_HANDS) && (get_eq_char (ch, WEAR_SECONDARY) != NULL))
		{
			send_to_char("You cannot wield a two-handed item while wielding a weapon in your off hand\n\r",ch);
			return;
		}
    ...
    if ( (obj->item_type == ITEM_ARROW) && ( CAN_WEAR(obj, ITEM_LODGE_LEG) ))
	{
		equip_char( ch, obj, WEAR_LODGE_LEG );
	    return;
	}

    if ( (obj->item_type == ITEM_ARROW) && ( CAN_WEAR(obj, ITEM_LODGE_ARM) ))
	{
		equip_char( ch, obj, WEAR_LODGE_ARM );
	    return;
	}

    if ( (obj->item_type == ITEM_ARROW) && ( CAN_WEAR(obj, ITEM_LODGE_RIB) ))
	{
...
void do_sacrifice( CHAR_DATA *ch, char *argument )
{
    ...
    {
	act( "$n offers $mself to Admin, who graciously declines.",
	    ch, NULL, NULL, TO_ROOM );
	send_to_char(
	    "Admin appreciates your offer and may accept it later.\n\r", ch );
	return;
    }
    ...
    if (silver == 1)
        send_to_char(
	    "Admin gives you one silver coin for your sacrifice.\n\r", ch );
    else
    {
	sprintf(buf,"Admin gives you %d silver coins for your sacrifice.\n\r",
		silver);
	send_to_char(buf,ch);
    }
    ...
    act( "$n sacrifices $p to Admin.", ch, obj, NULL, TO_ROOM );
...
void do_steal( CHAR_DATA *ch, char *argument )
{
	...
	{
		stc("{RThey're in a safe room you piece of crap!{x",ch);
		return;
	}
...
void do_list( CHAR_DATA *ch, char *argument )
{
	...
	for ( obj = keeper->carrying; obj; obj = obj->next_content )
	{
		...
		{
		    found = TRUE;
		    send_to_char( "{D[{GLvl	{YPrice	{BQty{D] {WItem{x\n\r", ch );
		}

		if (IS_OBJ_STAT(obj,ITEM_INVENTORY))
		    sprintf(buf,"{D[{G%3d {Y%5d {B-- {D] {W%s{x\n\r",
		...
		else
		{
		    ...
		    sprintf(buf,"{D[{G%2d {Y%5d {B%2d {D] {W%s{x\n\r",
...
/* wear object as a secondary weapon */
{
	...
	if (obj->item_type != ITEM_WEAPON)
	{
		stc("You must wield a weapon.\n\r",ch);
		stc("{RThanks to Asura for letting me know{x\n\r",ch);
		return;
	}
	...
	if (IS_WEAPON_STAT(obj,WEAPON_DOMINANT_HAND)
		&& (ch->class != 4)
		&& (ch->level <= LEVEL_HERO))
	{
		stc("Only Legionaires can wield two dominant handed weapons.\n\r",ch);
		return;
	}

	wield = get_eq_char(ch,WEAR_WIELD);

    /* check if the char is using a shield or a held item */
...
/* at last - the char uses the weapon */
	...
	{
		send_to_char("You cannot wield a secondary weapon while wielding a two-handed one\n\r",ch);
		return;
	}
...
{  
	...
	{
	       send_to_char ( "{WYou already have an arrow nocked!{x\n\r", ch );
	       return;
	}
    ...
    if ( quiver->value[0] > 0 )
    {
		...
		if (get_obj_index(quiver->value[1]) == NULL)
		{
			stc("ARROW VNUM NOT FOUND",ch);
			return;
		}
		arrow = create_object(get_obj_index(quiver->value[1]), 0);
		obj_to_char(arrow,ch);
		quiver->value[0] --;

		act( "{W$n draws $p{W from $P{W.{x", ch, arrow, quiver, TO_ROOM );
		act( "{WYou draw $p{W from $P{W.{x", ch, arrow, quiver, TO_CHAR );
...
void do_dislodge( CHAR_DATA *ch, char *argument )
{
	...
	damage( ch, ch, dam, gsn_bow, DAM_SLASH, TRUE,FALSE );
	return;
	}	
   ...
   else
   {
      ...
      if ((!IS_OBJ_STAT(obj ,ITEM_ANTI_EVIL) && IS_EVIL(ch)) ||
         ...
         if (obj->cost > 0 && obj->level > 0)
         {
            ...
            if (amount == 1)
            {
               sprintf(buf, "You receive {wone silver{x for your donation.\n\r");
               send_to_char(buf,ch);
            }
            else
            {
               sprintf( buf, "You receive {w%d silver{x for your donation.\n\r",amount);
               send_to_char(buf,ch);
            }
...
void do_display( CHAR_DATA *ch, char *argument)
{
	...
	act( "$n displays the contentes of $p to $N.", ch, obj, victim, TO_NOTVICT );
	act( "$n shows you the contents of $p.",   ch, obj, victim, TO_VICT    );
	act( "You show the contents of $p to $N.", ch, obj, victim, TO_CHAR    );
    show_list_to_char( obj->contains, victim, TRUE, TRUE );
    return;
...
void do_token(CHAR_DATA *ch, char * argument)
{
    ...
    if (argument[0] != '\0')
    {
		...
		if(!is_number(arg))
		{
			send_to_char("Ammount must be numeric in nature",ch);
			return;
		}
		if( cost <= 0)
		{
			sprintf(buf,"{WYour {CQP's{W have benn {Gadjusted{W by {Y%d{W.\n\r",cost);
			stc(buf,ch);
			ch->questpoints += cost;
			return;
		}

		if(( ch->questpoints) < (cost * 1.05))
		{
			send_to_char("You dont have that many QP's",ch);
			return;
		}
		sprintf(buf, "{WYou have created a {C%d {CQP {MQ{Gu{Re{Bs{Yt {Dt{Wo{wk{ye{bn{x.\n\r", cost);
		send_to_char(buf,ch);
		ch->questpoints -= (cost * 1.05);
		token = create_object( get_obj_index(OBJ_VNUM_TOKEN), 0 );
		sprintf( buf, "a {Y%d {CQP {MQ{Gu{Re{Bs{Yt {Dt{Wo{wk{ye{bn{x", cost );
		free_string( token->short_descr );
		token->short_descr = str_dup( buf ); /*Set Short Desc*/
		token->value[0] = cost;
