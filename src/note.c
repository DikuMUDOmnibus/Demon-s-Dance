...
void parse_note( CHAR_DATA *ch, char *argument, int type )
{
	...
	for ( pnote = *list; pnote != NULL; pnote = pnote->next )
	{
	    if ( is_note_to( ch, pnote ) )
	    {
		sprintf( buf, "{D[{M%3d%s{D] {G%s: {W%s{x\n\r",
...
/* i took out the note + stuff here*/
    if ( !str_prefix( arg, "edit" ) )
    {
		if (!ch->pnote)
		{
			stc("Set the subject and to list first please (help note for assistance)",ch);
			return;
		}
		string_append( ch, &ch->pnote->text );
		return;
    }
/*End of attempt to use editor*/
	...
	ch->pnote = NULL;
	send_to_char( "You have posted your current note. Thank you.\n\r", ch);
	return;
    }
