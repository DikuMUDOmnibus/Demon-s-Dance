...
void list_group_costs(CHAR_DATA *ch)
{
    ...
    sprintf(buf,"{D%-18s {R%-5s {D%-18s {R%-5s {D%-18s {R%-5s{x\n\r","group","cp","group","cp","group","cp");
    send_to_char(buf,ch);
	...
	&&  group_table[gn].rating[ch->class] > 0)
	{
	    sprintf(buf,"{D%-18s {R%-5d{x ",group_table[gn].name,
	...
	&&  skill_table[sn].rating[ch->class] > 0)
        {
            sprintf(buf,"{D%-18s {R%-5d{x ",skill_table[sn].name,
    ...
    sprintf(buf,"{DCreation points: {B%d{x\n\r",ch->pcdata->points);
    send_to_char(buf,ch);
    sprintf(buf,"{DExperience per level: {B%d{x\n\r",
...
void check_improve( CHAR_DATA *ch, int sn, bool success, int multiplier )
{
	...
	if (number_percent() < chance)
	{
		...
		sprintf(buf,"You have become better at %s! {D[{W%d%{D]{x\n\r",
