...
void save_mobile( FILE *fp, MOB_INDEX_DATA *pMobIndex )
{
    ...
    fprintf( fp, "%dd%d+%ld ",	pMobIndex->hit[DICE_NUMBER], 
	...
	{
		fprintf( fp, "%s ",		fwrite_flag( pMobIndex->teach_flags,  buf ) );		
	}
...
void save_guilds_new()
{
	...
	{
/*		pClan = clan_table[i];*/
		sprintf(buf,"%s saved",clan_table[i].who_name);
		wiznet(buf,NULL,NULL,WIZ_LINKS,0,0);
		save_guild(i);
	}
  
	wiznet("Guild Files saved in new format",NULL,NULL,WIZ_LINKS,0,0);
	return;        
}
...
{  
	...
	for (i=1; clan_lookup(clan_table[i].name) != 0; i++)
	{
		if ( !IS_SET(clan_table[i].flags, GUILD_DELETED) && clan_lookup(clan_table[i].name) != 0)
		{
			...
			fprintf(fp, "Rooms\t%d\t%d\t%d\n", 
				...
				clan_table[i].room[1],
				clan_table[i].room[2]);
			fprintf(fp,"Threshold %d\n",clan_table[i].threshold);
			fprintf(fp,"Prep %d\n",clan_table[i].prep_room);
			fprintf(fp,"Maze %d %d %d\n",
				clan_table[i].maze_begin,
				clan_table[i].maze_mid,
				clan_table[i].maze_end);
	...
	fprintf(fp, "\nEnd\n");
	fclose(fp);
	wiznet("Guild Files saved",NULL,NULL,WIZ_LINKS,0,0);
	return;        
}
