...
int regen_gain	args( ( CHAR_DATA *ch ) );
...
void honor_update     args( ( void ) );           /* Arena.c */
void smith_update     args( ( void ) );           /* Arena.c */
bool challenge_tme;
bool is_challenge;
...
void advance_level( CHAR_DATA *ch, bool hide )
{
    ...
    add_hp		= UMAX(  2, add_hp   ) + 50;
    add_mana	= UMAX(  2, add_mana ) + 25;
...
void gain_exp( CHAR_DATA *ch, int gain )
{
		...
		&& ch->exp >= exp_per_level(ch,ch->pcdata->points) * (ch->level+1) )
    {
		...
		ch->level += 1;
		set_new_title(ch);
		sprintf(buf,"{W%s gained level %d{x",ch->name,ch->level);
		do_info(ch,buf,ch);
...
void weather_update( void )
{
	...
	strcat( buf, "{YThe day has begun.{x\n\r" );
	break;

    case  6:
	weather_info.sunlight = SUN_RISE;
	strcat( buf, "{rThe sun rises in the east.{x\n\r" );
	break;

    case 19:
	weather_info.sunlight = SUN_SET;
	strcat( buf, "{yThe sun slowly disappears in the west.{x\n\r" );
	break;
...
{   
	...
	if (challenge_tme == 0 && IS_SET(ch->plr2,PLR2_CHALLENGER)
	&& !IS_SET(ch->in_room->vnum,30001))
	{
	...
	else if ( IS_AFFECTED(ch, AFF_DAGASHI) && ch != NULL)
	{
		AFFECT_DATA *poison;

		poison = affect_find(ch->affected,gsn_dagashi_poison);

		if (poison != NULL)
		{
			act( "$n's eyes grow feverish as $e is wracked with pain from the {BD{ra{Bg{ra{Bs{rh{Bi{x poisons{x", ch, NULL, NULL, TO_ROOM );
			send_to_char( "You feel your entrails burn as the {BD{ra{Bg{ra{Bs{rh{Bi{x poisons runs through your blood{x\n\r", ch );
			damage(ch,ch,poison->level*15,gsn_dagashi_poison,DAM_POISON,TRUE,FALSE);
		}

		if ( !IS_AFFECTED(ch, AFF_SLOW) && ch != NULL)
		{
	...
	if ( IS_AFFECTED2(ch, AFF_SHADOW_SLIP) && ch != NULL)
	{
		ch->move = (ch->move - (ch->move/10));
		stc("{WShadowslipping drains some of your stamina.....{x\n\r",ch);
	}
    }
...
void update_handler( )
{
	...
	static	int		pulse_honor;
	static	int		pulse_smith;
    ...
    if ( --pulse_point    <= 0 )
    {
	wiznet("{WTICK!{x",NULL,NULL,WIZ_TICKS,0,0);
...
int regen_gain( CHAR_DATA *ch )
{
	...
	if (IS_AFFECTED(ch,AFF_REGENERATION))
	{
		race_affect = race_table[ch->race].regen_affect;
		regen = (ch->level + (get_skill(ch,gsn_fast_healing))) * race_affect;
	}
	return regen;
}

void set_new_title( CHAR_DATA *ch )
{
	char buf[MSL];
	int i;
	...
	{
		/*Set new title*/
		for (i=0;i <= (MAX_CLASS * 12); i++)
		{
			if ( ch->class == title_table[i].class_num 
				&& ch->level == title_table[i].level )
			{
				switch (ch->sex)
				{
					default:			sprintf(buf," the %s",title_table[i].title_male); break;
					case SEX_MALE:		sprintf(buf," the %s",title_table[i].title_male); break;
					case SEX_FEMALE:	sprintf(buf," the %s",title_table[i].title_female); break;
					break;
				}
