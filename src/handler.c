...
int golem_type (const char *name)
{
    int type;
 
    for (type = 0; golem_table[type].name != NULL; type++)
    {
        if (LOWER(name[0]) == LOWER(golem_table[type].name[0])
        &&  !str_prefix(name,golem_table[type].name))
            return golem_table[type].type;
    }
 
    return GOLEM_HEART;
}

char *golem_name( int golem_type)
{
    int type;
 
    for (type = 0; golem_table[type].name != NULL; type++)
        if (golem_type == golem_table[type].type)
            return golem_table[type].name;
    return "heart";
}
...
int get_weapon_sn(CHAR_DATA *ch)
{
    ...
    {
        default :               sn = gsn_hand_to_hand;	break;
...
/* for returning weapon information (dual wielded item)*/
...
void reset_char(CHAR_DATA *ch)
{
	...
	for ( af = obj->pIndexData->affected; af != NULL; af = af->next )
		...
		case VRES_COLD:			ch->vres_cold		+= mod; break;
		case VRES_FIRE:			ch->vres_fire		+= mod; break;
		case VRES_ACID:			ch->vres_acid		+= mod; break;
		case VRES_NEGATIVE:		ch->vres_negative	+= mod; break;
		case VRES_HOLY:			ch->vres_holy		+= mod; break;
		case VRES_LIGHTNING:	ch->vres_lightning	+= mod; break;
		case VRES_PIERCE:		ch->vres_pierce		+= mod; break;
		case VRES_SLASH:		ch->vres_slash		+= mod; break;
		case VRES_BASH:			ch->vres_bash		+= mod; break;
		case VRES_POISON:		ch->vres_poison		+= mod; break;
		case VRES_ENERGY:		ch->vres_energy		+= mod; break;
		case VRES_DISEASE:		ch->vres_disease	+= mod; break;
		case VRES_MAGIC:		ch->vres_magic		+= mod; break;
		case VRES_WEAPON:		ch->vres_weapon		+= mod; break;
		case VRES_MENTAL:		ch->vres_mental		+= mod; break;
		case VRES_LIGHT:		ch->vres_light		+= mod; break;
		case VRES_SOUND:		ch->vres_sound		+= mod; break;
		}
		}
...
void locker_to_char( OBJ_DATA *obj, CHAR_DATA *ch )
{
    obj->next_content	 = ch->carrying;
    ch->carrying	 = obj;
    obj->carried_by	 = ch;
...
/*
 *Find an object by OBJ_DATA
 */

OBJ_DATA *get_objdata_list( CHAR_DATA *ch, OBJ_DATA *obj, OBJ_DATA *list )
{
    int number = 0;
	OBJ_DATA *target;
    int count;
    ...
    count  = 0;
    for ( target = list; target != NULL; target = list->next_content )
    {
		if ( can_see_obj( ch, target ) && (target = obj) )
		{
			if ( ++count == number )
			return obj;
		}
...
OBJ_DATA *create_money( int gold, int silver )
{
	...
	obj->weight		= gold / 1000 + silver / 1000;
    }
...
/* RT changed so that WIZ_INVIS has levels */
	...
	/*shadowsliping*/
	if ( IS_AFFECTED2(victim, AFF_SHADOW_SLIP) 
		&& !IS_AFFECTED2(ch,AFF_MYSTIC_SIGHT))
	return FALSE;
...
char *affect_loc_name( int location )
{
	...
	case VRES_COLD:			return "vres cold";
	case VRES_FIRE:			return "vres fire";
	case VRES_ACID:			return "vres acid";
	case VRES_NEGATIVE:		return "vres negative";
	case VRES_HOLY:			return "vres holy";
	case VRES_LIGHTNING:	return "vres lightning";
	case VRES_PIERCE:		return "vres pierce";
	case VRES_SLASH:		return "vres slash";
	case VRES_BASH:			return "vres bash";
	case VRES_POISON:		return "vres poisons";
	case VRES_ENERGY:		return "vres energy";
	case VRES_DISEASE:		return "vres disease";
	case VRES_MAGIC:		return "vres magic";
	case VRES_WEAPON:		return "vres weapon";
	case VRES_MENTAL:		return "vres mental";
	case VRES_LIGHT:		return "vres light";
	case VRES_SOUND:		return "vres sound";
...
char *affect_bit_name( int vector )
{
	...
	if ( vector & AFF_DAGASHI        ) strcat( buf, " dagashi"        );
...
char *affect2_bit_name( int vector )
{
	...
	if ( vector & AFF_PHALANX   ) strcat( buf, " phalanx"			);
    if ( vector & AFF_FORTIFY   ) strcat( buf, " fortify"			);    
	if ( vector & AFF_SHIELD_WALL   ) strcat( buf, " shield_wall"   );
	if ( vector & AFF_SHADOW_SLIP   ) strcat( buf, " shadow_slip"	);
	if ( vector & AFF_HOLY_ARMOR  ) strcat( buf, " holy_armor"		);
	if ( vector & AFF_MYSTIC_SIGHT  ) strcat( buf, " mystic_sight"		);
    return ( buf[0] != '\0' ) ? buf+1 : "none";
}
...
char *act_bit_name( int act_flags )
{
	...
	if (act_flags & ACT_MULTI_HIT	) strcat(buf, " multi_hit");
	...
	if (act_flags & ACT_QUESTMASTER	) strcat(buf, " quest_master");
	if (act_flags & ACT_UPDATE_ALWAYS) strcat(buf," update_always");
	if (act_flags & ACT_NO_QUEST) strcat(buf," no_quest");
    }
    else
    {
	strcat(buf," player");
	if (act_flags & PLR_AUTOGOLEM	) strcat(buf, " autogolem");
...
char *wear_bit_name(int wear_flags)
{
	...
	if (wear_flags & ITEM_WEAR_FACE	)	strcat(buf, " face");
	if (wear_flags & ITEM_WEAR_NOCKED	) strcat(buf, " nocked");
...
char *part_bit_name(int part_flags)
{
	...
	if (part_flags & PART_FACE		) strcat(buf, " face");
...
char *weapon_bit_name(int weapon_flags)
{
    ...
    if (weapon_flags & WEAPON_DAGASHI	)		strcat(buf, " dagashi");
	if (weapon_flags & WEAPON_OFF_HAND	)		strcat(buf, " offhand");
	if (weapon_flags & WEAPON_DOMINANT_HAND )	strcat(buf, " dominanthand");
...
char *teach_bit_name(int teach_flags)
{
    static char buf[512];

    buf[0] = '\0';

	if (teach_flags & TEACH_NONE			) strcat(buf, " none"		);
    if (teach_flags & TEACH_MULTI_HIT		) strcat(buf, " multi_hit"	);
    if (teach_flags & TEACH_LEGIONARE_1		) strcat(buf, " legionare_1");
    if (teach_flags & TEACH_LEGIONARE_2		) strcat(buf, " legionare_2");
    if (teach_flags & TEACH_SENSEI_1		) strcat(buf, " sensei_1"	);
    if (teach_flags & TEACH_SENSEI_2		) strcat(buf, " sensei_2"	);
    if (teach_flags & TEACH_DRACONIAN_1		) strcat(buf, " draconian_1");
    if (teach_flags & TEACH_DRACONIAN_2		) strcat(buf, " draconian_2");
    if (teach_flags & TEACH_TAO_1			) strcat(buf, " tao_1"		);
    if (teach_flags & TEACH_TAO_2			) strcat(buf, " tao_2"		);
    if (teach_flags & TEACH_ZEALOT_1		) strcat(buf, " zealot_1"	);
    if (teach_flags & TEACH_ZEALOT_2		) strcat(buf, " zealot_2"	);
    if (teach_flags & TEACH_DAGASHI_1		) strcat(buf, " dagashi_1"	);
    if (teach_flags & TEACH_DAGASHI_2		) strcat(buf, " dagashi_2"	);
    if (teach_flags & TEACH_MULTI_CAST		) strcat(buf, " multi_cast"	);

    return ( buf[0] != '\0' ) ? buf+1 : "none";
}


int check_vres(CHAR_DATA *ch, int dam_type)
{
    int bit;
    ...
    /* set bits to check -- vres == damtype */
    switch (dam_type)
    {
	case(DAM_BASH):			bit = ch->vres_bash;		break;
	case(DAM_PIERCE):		bit = ch->vres_pierce;		break;
	case(DAM_SLASH):		bit = ch->vres_slash;		break;
	case(DAM_FIRE):			bit = ch->vres_fire;		break;
	case(DAM_COLD):			bit = ch->vres_cold;		break;
	case(DAM_LIGHTNING):	bit = ch->vres_lightning;	break;
	case(DAM_ACID):			bit = ch->vres_acid;		break;
	case(DAM_POISON):		bit = ch->vres_poison;		break;
	case(DAM_NEGATIVE):		bit = ch->vres_negative;	break;
	case(DAM_HOLY):			bit = ch->vres_holy;		break;
	case(DAM_ENERGY):		bit = ch->vres_energy;		break;
	case(DAM_MENTAL):		bit = ch->vres_mental;		break;
	case(DAM_DISEASE):		bit = ch->vres_disease;		break;
	case(DAM_LIGHT):		bit = ch->vres_light;		break;
	case(DAM_SOUND):		bit = ch->vres_sound;		break;
